<?php
  ...
  if(isset($_POST["submit2"]))
  {
    ...
    $sql = "INSERT INTO production_detail
            (production_nama, production_deadline, production_detail, production_id_employee, production_id_client, production_id_treatment_dtl)
            VALUES ('$itemName', '$deadline', '$detail', '$pegawaii', '$klienn', '$jenis')";
    $query = mysqli_query(mySqlConnection(), $sql);
          ...
          <div class="div-forms p-5">
            ...
            <form method="post">
              ...
              <select class="form-select" aria-label="Default select example" name="jenis">
                ...
                <option value="<?php echo $row["treatment_id"]; ?>"><?php echo $row["treatment_name"]; ?></option>
                <?php } ?>
              </select>
              <!-- Hidden Date -->
              <div class="mb-3">
                <input type="hidden" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo date("Y-m-d", strtotime($newDate . " + 1 Day")) ?>" autocomplete="off" name="deadline" />
              </div>
              <!-- Tampilan -->
              <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Deadline</label>
                <input type="date" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo date("Y-m-d", strtotime($newDate . " + 1 Day")) ?>" autocomplete="off" disabled/>
              </div>
              <div class="mb-3">
                <div class="form-floating">
                  <textarea class="form-control" placeholder="Detail Barang" id="floatingTextarea" name="detail"></textarea>
                  <label for="floatingTextarea">Detail</label>
                </div>
              </div>
              <!-- Hidden Pegawai -->
              <div class="mb-3">
                <input type="hidden" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo $_SESSION["usernameEmp"] ?>" autocomplete="off" name="namaPegawai" />
              </div>
              <!-- Tampilan -->
              <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Pegawai</label>
                <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo $_SESSION["usernameEmp"] ?>" autocomplete="off" disabled/>
              </div>
              <!-- Hidden -->
              <div class="mb-3">
                <input type="hidden" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo $wa; ?>" autocomplete="off" name="klienWA" />
                <input type="hidden" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo $nama; ?>" autocomplete="off" name="klien" />
              </div>
              <!-- Tampilan -->
              <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Client</label>
                <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" value="<?php echo $nama; ?>" autocomplete="off" disabled/>
              </div>
